...
def glue_execution_timestream_metrics_summary(
    ...
    TABLE_NAME = AWSNaming.TimestreamMetricsTable(
        stack_obj_for_naming, SettingConfigResourceTypes.GLUE_CRAWLERS
    )
...
def test_alerts(test_results_messages):
    ...
    cnt_glue_error_messages = len(
        msqchk.subject_contains_all(["glue_crawlers :", "Failed"])
    )
    cnt_glue_all_messages = len(msqchk.subject_contains_all(["glue_crawlers :"]))

    assert (
        cnt_glue_error_messages == 1
    ), "There should be exactly one glue crawler error message"
    assert (
        cnt_glue_all_messages == 1
    ), "There should be exactly one glue crawler message"
...
def test_timestream_records(glue_execution_timestream_metrics_summary):
    ...
    assert (
        executions == "2"
    ), "There should be exactly 2 executions. One for each glue crawler."
...
def test_digest_message(test_results_messages, config_reader, stack_obj_for_naming):
    ...
    glue_crawler_names = config_reader.get_names_by_resource_type(
        SettingConfigResourceTypes.GLUE_CRAWLERS, stack_obj_for_naming
    )
    ...
    for glue_crawler_name in glue_crawler_names:
        assert (
            glue_crawler_name in message_body
        ), f"There should be mention of {glue_crawler_name} glue crawler"
