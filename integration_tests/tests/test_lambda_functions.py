...
def lambda_execution_timestream_metrics_summary(
...
@pytest.fixture(scope="session")
def lambda_error_timestream_metrics_summary(
    region, start_epochtimemsec, stack_obj_for_naming
):
    """
...
def test_alerts(test_results_messages):
    ...
    cnt_lambda_error_messages = len(
        msqchk.subject_contains_all(["lambda_functions :", "FAILED"])
    )
    cnt_lambda_all_messages = len(msqchk.subject_contains_all(["lambda_functions :"]))

    assert (
        cnt_lambda_error_messages == 1
    ), "There should be exactly one lambda function error message"
    assert (
        cnt_lambda_all_messages == 1
    ), "There should be exactly one lambda function message"


@pytest.mark.skip
def test_timestream_records(
    lambda_execution_timestream_metrics_summary, lambda_error_timestream_metrics_summary
):
    """
...
def test_digest_message(test_results_messages, config_reader, stack_obj_for_naming):
    ...
    lambda_function_names = config_reader.get_names_by_resource_type(
        SettingConfigResourceTypes.LAMBDA_FUNCTIONS, stack_obj_for_naming
    )

    assert (
        len(lambda_function_names) > 0
    ), "There should be lambda functions in testing scope"

    for lambda_function_name in lambda_function_names:
        assert (
            lambda_function_name in message_body
        ), f"There should be mention of {lambda_function_name} lambda function"
