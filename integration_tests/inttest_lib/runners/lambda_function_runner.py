...
class LambdaFunctionRunner(BaseResourceRunner):
    def __init__(self, resource_names, region_name):
        ...
        self.logs_manager = CloudWatchManager(boto3.client("logs", region_name=region_name))
        self.function_runs = {}
    ...
    def initiate(self):
        for lambda_function_name in self.resource_names:
            ...
            print(f"Invoked {lambda_function_name} with request ID {self.function_runs[lambda_function_name]}")
    ...
    def _invoke_lambda_async(
        ...
        try:
            ...
            if not request_id:
                raise LambdaFunctionRunnerException(f"RequestId not found in response for {lambda_function_name}")
            return request_id
    ...
    def _is_lambda_completed(self, lambda_function_name, request_id, start_time):
        ...
        try:
            results = self.logs_manager.query_logs(log_group_name, query_string, start_time, datetime.now())
            if results:
                print(f"Lambda function {lambda_function_name} with request ID {request_id} has completed.")
                return True
