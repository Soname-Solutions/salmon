    ...
    def initiate(self):
        for app_name, script_paths in self.emr_resources_data.items():
            ...
            for script_path in script_paths:
                ...
                self.job_runs[script_path] = {
                    "app_id": app_id,
                    "job_run_id": job_run_id,
                }
                print(
                    f"Started EMR Serverless job {script_path} with run ID {job_run_id}"
                )
    ...
    def await_completion(self, poll_interval=10):
        while True:
            ...
            for script_path, job_run_data in self.job_runs.items():
                ...
                job_run: EMRJobRunData = self.emr_manager.get_job_run(
                    app_id=app_id, run_id=job_run_id
                )

                print(
                    f"Job {script_path} with run ID {job_run_id} is in state {job_run.state}. "
                    + f"ResourceUtilization populated: {job_run.is_ResourceUtilization_populated}"
                )
                if not (job_run.is_final_state):
                    all_completed = False
