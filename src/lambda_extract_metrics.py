...
def get_service_client(account_id, region, role_name, aws_client_name):
    ...
    try:
        ...
        logger.info(f"Created {aws_client_name} client, role: {extract_metrics_role_arn}")
        return client
...
def process_all_resources_by_type(
    ...
    for name in resource_names:
        resource_name = name
        monitored_env = monitored_environment_name
        logger.info(
...
def lambda_handler(event, context):
    ...
    for attr_name in content:
        ...
        if isinstance(attr_value, list) and attr_name in SettingConfigs.RESOURCE_TYPES:
            ...
            #logger.info(json.dumps(attr_value, indent=4, default=str))

            data = attr_value
            # sorting so we can process all resources in each env in one shot
            ...
            for monitored_environment_name, group in groupby(
                ...
                logger.info(
                    f"Processing env: {monitored_environment_name}, resources: {resource_names}"
                )
                process_all_resources_by_type(
                    monitored_environment_name, resource_type, resource_names, settings, iam_role_name
                )
