...
def test_two_completed_jobs_records_integrity(boto3_client_creator):
    ...
    with patch(
        "lib.metrics_extractor.glue_jobs_metrics_extractor.GlueManager.get_job_runs"
    ) as mocked_get_job_runs:
...
def test_dpu_seconds_calculated(boto3_client_creator):
    ...
    with patch(
        ...
        assert str(dpu_rec_1) == str(
            dpu_rec_1_expected
        ), "Record 1: DPU seconds should be taken from output"

        dpu_rec_2 = get_measure_value(records[1], "dpu_seconds")
        dpu_rec_2_expected = (
            float(JOB_RUN_COMPLETED_WITHOUT_DPU.MaxCapacity)
            * JOB_RUN_COMPLETED_WITHOUT_DPU.ExecutionTime
        )
        assert str(dpu_rec_2) == str(
            dpu_rec_2_expected
        ), "Record 2: DPU seconds should be calculated properly"
