...
@pytest.fixture(
    params=[
        (
            "2024-02-05 07:46:42.335000000",
            datetime(2024, 2, 5, 7, 46, 42, 335000, tzinfo=timezone.utc),
        ),
        # Datetime with fractional seconds (but 6 digits)
        (
            "2024-02-05 07:46:42.335000",
            datetime(2024, 2, 5, 7, 46, 42, 335000, tzinfo=timezone.utc),
        ),
        # Datetime with full fractional seconds ending in zeros
        (
            "2024-02-03 19:36:52.000000000",
            datetime(2024, 2, 3, 19, 36, 52, 0, tzinfo=timezone.utc),
        ),
    ]
)
...
def test_str_utc_datetime_to_datetime_with_fixture(input_data):
    ...
    ), "The converted datetime does not match the expected datetime."
...
def test_utc_timezone_assignment():
    ...
    result = str_utc_datetime_to_datetime(input_datetime_str)
    assert (
        result.tzinfo is not None and result.tzinfo == timezone.utc
    ), "The timezone is not correctly assigned as UTC."
