...
class GlueDQRunner(BaseResourceRunner):
    def __init__(
        ...
        resource_names,
        region_name,
        started_after_epoch_msec,
        stack_obj_for_naming,
    ):
        super().__init__(resource_names, region_name)
        self.client = boto3.client("glue", region_name=region_name)
        self.started_after = epoch_seconds_to_utc_datetime(
            started_after_epoch_msec / 1000 - 1
        )
    ...
    def await_completion(self, poll_interval=10):
        while True:
            ...
            ruleset_names = []
            if result_ids:
                response_2 = self.client.batch_get_data_quality_result(
