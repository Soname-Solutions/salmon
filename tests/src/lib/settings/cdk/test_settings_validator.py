...
def test_validate_good_config(config_path_settings_tests):
...
def test_validate_bad_json(config_path_settings_tests):
    config_path = os.path.join(config_path_settings_tests, "config8_bad_json")
    with pytest.raises(json.decoder.JSONDecodeError):
        settings = Settings.from_file_path(config_path)
        result = validate(settings)


def test_validate_bad_json_schema(config_path_settings_tests):
    config_path = os.path.join(config_path_settings_tests, "config9_bad_json_schema")
    settings = Settings.from_file_path(config_path)
...
def test_validate_bad_json_duplicate_env_names(config_path_settings_tests):
    config_path = os.path.join(
        config_path_settings_tests, "config10_bad_json_schema_dup_envs"
    )
    settings = Settings.from_file_path(config_path)
