...
TIMESTREAM_QUERY_RUNNER_CLASS_NAME = "lib.metrics_extractor.base_metrics_extractor.TimeStreamQueryRunner"
...
def test_get_last_update_time_empty(boto3_client_creator):
    ...
    with patch(f"{TIMESTREAM_QUERY_RUNNER_CLASS_NAME}.is_table_empty", return_value=True):
...
def test_get_last_update_time_non_empty(boto3_client_creator):
    ...
    with patch(f"{TIMESTREAM_QUERY_RUNNER_CLASS_NAME}.is_table_empty", return_value=False), \
        patch(f"{TIMESTREAM_QUERY_RUNNER_CLASS_NAME}.execute_scalar_query_date_field", return_value="TestData"):
        last_update_time = extractor.get_last_update_time(None)
