...
def test1():
    ...
    with patch("lib.notification_service.sender.sns.SnsTopicPublisher") as MockAwsSnsManager:
        sender = AwsSnsSender(delivery_method=delivery_method, message=message, recipients=recipients)
        sender.pre_process()
        sender.send()
